...
function welcome (name="menna",salary=5000){
    console.log(`you name is menna${name}your salary${salary}`);
}
welcome();
...
let person={
    ...
    age:29,
   getBirthDay(){
       let d=newDate().getBirthDay;
       alert(d-this.age);
   },
   getSalary(){
       getTaxs=()=>(this.salary*14)/100;
...
for(let [x,y] of objectt ){
    console.log(`name${name}salary${salary}`)
}
 let newObject=new Map(object.entries(objectt))
